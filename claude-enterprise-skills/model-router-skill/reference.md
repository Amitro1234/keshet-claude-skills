...
Detailed decision logic, protocols, and examples for `model-router-skill/SKILL.md`.
Load this file only when applying the router in detail — the core SKILL.md
covers the tier table and the non-negotiable rules on their own.
...
**Keshet cost context:** A team of 20 Builders defaulting to Sonnet for every
operation spends ~$130K/year on API. Consistent routing reduces that to
~$95K — a saving of ~$35K without reducing quality (see `_shared/model-tiers.md`
for current per-token rates behind this estimate).
