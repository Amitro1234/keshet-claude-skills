...
CASES = [
    ...
    ("rm -rf /home (system top-level dir wipe)",
     {"tool_name": "Bash", "tool_input": {"command": "rm -rf /home"}}, "block"),
    ("rm -rf /etc (system top-level dir wipe)",
     {"tool_name": "Bash", "tool_input": {"command": "rm -rf /etc/"}}, "block"),
    ("rm -rf ~ (home dir wipe)",
     {"tool_name": "Bash", "tool_input": {"command": "rm -rf ~"}}, "block"),
    ("rm -rf . (cwd wipe)",
     {"tool_name": "Bash", "tool_input": {"command": "rm -rf ."}}, "block"),
    ("rm -rf $HOME (home dir wipe via var)",
     {"tool_name": "Bash", "tool_input": {"command": "rm -rf $HOME"}}, "block"),
    ("fork bomb",
    ...
    ("rm -rf ../sibling-project (safe, scoped delete, must not false-positive on '..')",
     {"tool_name": "Bash", "tool_input": {"command": "rm -rf ../sibling-project"}}, "allow"),
    ...
    # --- PowerShell tool coverage (Windows Builder sessions) ---------------
    ("Get-Content .env (PowerShell secret content)",
     {"tool_name": "PowerShell", "tool_input": {"command": "Get-Content .env"}}, "block"),
    ("Select-String for API_KEY (PowerShell)",
     {"tool_name": "PowerShell", "tool_input": {"command": "Select-String -Pattern API_KEY -Path .\\config.py"}}, "block"),
    ("Remove-Item -Recurse -Force C:\\ (PowerShell destructive)",
     {"tool_name": "PowerShell", "tool_input": {"command": "Remove-Item -Recurse -Force C:\\"}}, "block"),
    ("Remove-Item -Force -Recurse (flag order swapped)",
     {"tool_name": "PowerShell", "tool_input": {"command": "Remove-Item -Force -Recurse .\\project"}}, "block"),
    ("Format-Volume (PowerShell disk format)",
     {"tool_name": "PowerShell", "tool_input": {"command": "Format-Volume -DriveLetter D"}}, "block"),
    ("Remove-Item node_modules -Recurse -Force (PowerShell, scoped -- still blocked, human must run)",
     {"tool_name": "PowerShell", "tool_input": {"command": "Remove-Item .\\node_modules -Recurse -Force"}}, "block"),
    ("Get-ChildItem normal listing (PowerShell, safe)",
     {"tool_name": "PowerShell", "tool_input": {"command": "Get-ChildItem ."}}, "allow"),
    ("git status via PowerShell (safe)",
     {"tool_name": "PowerShell", "tool_input": {"command": "git status"}}, "allow"),
