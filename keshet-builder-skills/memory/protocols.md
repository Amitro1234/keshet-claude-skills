# Memory Skill — Protocols and Templates

Full step-by-step protocols (Standard / Cowork / In-Conversation), decision
entry format, session-end procedure, and file templates for
`memory/SKILL.md`. Load this file once platform detection has picked a
protocol — the core SKILL.md covers triggers and the platform-detection
...
If the project is git-backed, memory files must be committed to keep them
shared and backed up. If not (e.g. a Cowork connected folder with no git —
see Cowork Protocol above), skip this section: remind the user the files
exist only in the folder and won't have version history, but they still
persist across sessions as plain files.
